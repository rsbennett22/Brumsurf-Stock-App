        ...
        if(stockType=='boot' or stockType=='glove' or stockType=='hood'):
            if(stockType=='boot'):
                ...
                try:
                    ...
                    pk = bootMade.pk
                    bootMade.url = 'http://192.168.0.65:8000/detail/'+str(pk)
                    bootMade.save()
                ...
                try:
                    ...
                    pk = gloveMade.pk
                    gloveMade.url = 'http://192.168.0.65:8000/detail/'+str(pk)
                    gloveMade.save()
                    #Load accessory details page
                    return accessoryDetail(request, pk)
            else:
                ...
                try:
                    ...
                    pk = hoodMade.pk
                    hoodMade.url = 'http://192.168.0.65:8000/detail/'+str(pk)
                    hoodMade.save()
                    #Load accessory details page
                    return accessoryDetail(request, pk)
            ...
            if(checkForQR(fileName)):
                ...
                if(stockType=='wetsuit'):
                    ...
                    newWetsuit.url='http://192.168.0.65:8000/detail/'+stockType+'&'+str(number)
                    ...
                    newBoard.url='http://192.168.0.65:8000/detail/'+stockType+'&'+str(number)
                    newBoard.save()
                    print(bcolors.OKBLUE+"Successfully created a new "+stockType+" instance!"+bcolors.ENDC)
                    return itemDetail(request, stockType, number)
...
def generateQRCode(stockType, brand, gender, size, number, fileName):
    ...
    qrData = 'http://192.168.0.65:8000/'+stockType+'/'+brand+'&'+gender+'&'+str(size)+'&'+str(number)
