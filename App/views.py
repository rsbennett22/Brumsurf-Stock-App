...
def wetsuit(request, brand, size, gender, number):
    ...
    return render(request, 'App/wetsuit.html', {'stockType' : 'Wetsuit', 'brand' : brand, 'size' : size, 'gender' : gender, 'number' : number, 'fileName' : fileName, 'qrImg' : qrImg})
...
def generateWetsuitQR(request, brand, size, gender, number):
    ...
    return render(request, 'App/redirectToWetsuitPage.html', {'stockType' : 'wetsuit', 'size' : size, 'brand' : brand ,'number' : number, 'gender' : gender, 'qrImg' : qrImg, 'fileName' : fileName})